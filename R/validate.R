...
validate <- function(data_raw, data_mod, dir_proj, p_dots, fit_obj, iter) {
  ...
  p_resid_list <- purrr::map(var_plot_vec, function(var) {
    if (var == ".no_exp_var") {
      ...
      data_res_std_dens <- tibble::tibble(
        x = dens_res_std$x, y = dens_res_std$y,
        type = "Std residuals"
      )
      data_norm <- tibble::tibble(
        x = seq(min(data_res_std_dens$x),
  ...
  break_list <- map(var_exp_vec, function(var) {
    if (is.character(data_mod[[var]]) || is.logical(data_mod[[var]])) {
      return(unique(data_mod[[var]]))
    }
    if (is.factor(data_mod[[var]])) {
  ...
  kw_obj <- map_df(var_exp_list, function(var_exp) {
    ...
    out_tbl <- try(tibble::tibble(
      var = var_print, stat = kw_obj$statistic, df = length(unique(grp_vec)) - 1,
      p = kw_obj$p.value
    ))
    if (class(out_tbl) == "try-error") {
      return(tibble(var = NA_character_, stat = NA_real_, df = NA_integer_))
    }
    out_tbl
  })

  pipeline::save_objects(
    obj_list = list(kw = kw_obj),
    dir_proj = dir_save,
    empty = FALSE
  )
  invisible(TRUE)
