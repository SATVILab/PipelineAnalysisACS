.get_np_results <- function(data_mod, p_dots) {
  ...
  if(names(p_dots$var_exp_spline) == "tfmttb") {
    ...
    results_tbl_np <- results_tbl_np %>%
      dplyr::bind_rows(
        tibble::tibble(var = "Progressor",
                       p = p_prog)
      )

    cat_vec_int <- purrr::map_chr(seq_len(nrow(data_mod)), function(i) {
      if(data_mod$Progressor[[i]] == "no") return("no")
      cat_vec_tfmttb[i]
    })
    ...
    results_tbl_np <- results_tbl_np %>%
      dplyr::bind_rows(
        tibble::tibble(var = "Progressor; tfmttb",
                       p = p_prog_tfmttb)
      )
    ...
    if (!is.null(p_dots$var_int)) {
      ...
      results_tbl_np <- results_tbl_np %>%
        dplyr::bind_rows(
          tibble::tibble(var = paste0(p_dots$var_int, collapse = "; "),
                         p = p)
        )
    }
    ...
    if (!is.null(p_dots$var_exp)) {
      ...
      results_tbl_np <- results_tbl_np %>%
        dplyr::bind_rows(
          tibble::tibble(var = p_dots$var_exp,
                         p = p)
        )
    }
...
if (!is.null(p_dots$var_exp_s)) {
  ...
  results_tbl_np <- results_tbl_np %>%
    dplyr::bind_rows(
      tibble::tibble(var = names(var_exp_s),
                     p = p)
    )
}
}
