...
get_pipeline_df <- function(dir_base, dir_sub = "", fn, df_cols = NULL, obj_function){
  ...
  if(!obj_ind){
    ...
    out_list <- map_df(dir_sub_list, function(x) get_pipeline_df(dir_base = dir_base, dir_sub = x,
                                                                 fn = fn, df_cols = df_cols))
  } else{
    obj <- readRDS(file.path(dir_base_sub, fn))
    mat_var <- matrix(rep(dir_sub, nrow(obj)), byrow = TRUE,
                      ncol = length(dir_sub))
